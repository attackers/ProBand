...
- (SectionModel *)dailySectionModelForDate:(NSString *)date
{
    ...
    if (data.count > 0)
    {
        ...
        NSMutableArray *valueArray = [NSMutableArray array];
        for (int i = 0; i < stepArray.count-1; i ++)
        {
            NSString *number = stepArray[i];
            ...
            else//否则为睡眠时间:0,连续的情况需要避免,结束时间点需要加上最后一个点
            {
                ...
                if (i > startPoint && [str intValue]>threshold && [stepArray[i+1] intValue]<=threshold)
                {
                    endPoint = i;//i-1
        ...
        int count = stepArray.count;
        if ([[stepArray lastObject] intValue]>threshold || [stepArray[count-1] intValue]>threshold)
        {
