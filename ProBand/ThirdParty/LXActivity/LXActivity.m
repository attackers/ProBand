...
- (void)creatButtonsWithTitle:(NSString *)title cancelButtonTitle:(NSString *)cancelButtonTitle shareButtonTitles:(NSArray *)shareButtonTitlesArray withShareButtonImagesName:(NSArray *)shareButtonImagesNameArray
{
    ...
    if (title) {
        ...
        lineLayer.frame = CGRectMake(0, CGRectGetMaxY(titleLabel.frame), self.frame.size.width, 1);
        lineLayer.backgroundColor = [UIColor colorWithRed:225/255.0 green:225/255.0 blue:225/255.0 alpha:1.0].CGColor;
        ...
        if (shareButtonImagesNameArray.count > 0) {
            ...
            for (int i = 1; i < shareButtonImagesNameArray.count+1; i++) {
                ...
                if (self.isHadTitle == YES) {
                    [shareButton setFrame:CGRectMake(SCREEN_WIDTH/2+70*(i-2)+13, CGRectGetMaxY(lineLayer.frame)+20, SHAREBUTTON_WIDTH, SHAREBUTTON_HEIGHT)];
                }
                else{
                    [shareButton setFrame:CGRectMake(SCREEN_WIDTH/2+70*(i-2)+13, CGRectGetMaxY(lineLayer.frame)+20, SHAREBUTTON_WIDTH, SHAREBUTTON_HEIGHT)];
                }
    ...
    if (shareButtonTitlesArray) {
        if (shareButtonTitlesArray.count > 0 && shareButtonImagesNameArray.count > 0) {
            for (int j = 1; j < shareButtonTitlesArray.count+1; j++) {
                ...
                //有Title的时候
                if (self.isHadTitle == YES) {
                    [shareLabel setFrame:CGRectMake(SCREEN_WIDTH/2+70*(j-2)+13, CGRectGetMaxY(lineLayer.frame)+80, SHARETITLE_WIDTH, SHARETITLE_HEIGHT)];
                }
    ...
    //画一道线
    CALayer *nilLayer = [[CALayer alloc]init];
    nilLayer.frame = CGRectMake(0, CGRectGetMaxY(lineLayer.frame)+110, self.frame.size.width, 10);
    nilLayer.backgroundColor = [UIColor colorWithRed:0 green:0 blue:0 alpha:0.2].CGColor;
    [self.backGroundView.layer addSublayer:nilLayer];
...
{
    UIButton *cancelButton = [[UIButton alloc] initWithFrame:CGRectMake(0, BUTTON_INTERVAL_HEIGHT, self.frame.size.width, BUTTON_HEIGHT+20)];
...
//    cancelButton.layer.borderColor = BUTTON_BORDER_COLOR;
    ...
    [cancelButton setTitleColor:TEXT_COLOR forState:UIControlStateNormal];
...
- (UILabel *)creatShareLabelWithColumn:(int)column andLine:(int)line
{
    ...
    shareLabel.textColor = TEXT_COLOR;
    return shareLabel;
}

- (UILabel *)creatTitleLabelWith:(NSString *)title
{
    UILabel *titlelabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 5, SCREEN_WIDTH, TITLE_HEIGHT+20)];
