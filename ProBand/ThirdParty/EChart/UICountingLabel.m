...
{
    [self countFrom:value to:endValue withDuration:0.05f];
}
...
-(void)updateValue:(NSTimer*)timer
{
    ...
    else
    {
        ...
        {
            NSString *str = [NSString stringWithFormat:@"%@%%",[NSString stringWithFormat:self.format,(int)value]];
            NSMutableAttributedString *attributedStr = [[NSMutableAttributedString alloc]initWithString:str];
            [attributedStr addAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:60]} range:NSMakeRange(0, str.length -1)];
            [attributedStr addAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:16]} range:NSMakeRange(str.length - 1, 1)];
        ...
        {
            NSString *str = [NSString stringWithFormat:@"%@%%",[NSString stringWithFormat:self.format,value]];
            NSMutableAttributedString *attributedStr = [[NSMutableAttributedString alloc]initWithString:str];
            [attributedStr addAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:60]} range:NSMakeRange(0, str.length -1)];
            [attributedStr addAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:16]} range:NSMakeRange(str.length - 1, 1)];
            self.attributedText = attributedStr;
